...
`speaker`/`mentioned` 的 `name` 只填**单个人物的名字/称呼**，严格禁止：
- 带家庭/集体词缀：「老保一家」「他们全家」「张三两口子」→ 只取核心人名「老保」「张三」
- 带称谓造句：「一个叫李四的人」「王五先生来访」→ 只取「李四」「王五」
- 代词或非人名：「他们」「大家」「有人」「所有人」「一对夫妻」→ 不要建人物
- 超过 8 个字的名字一律不要建人物
拿不准是不是人名时，宁可不建（后续节点还有硬校验兜底）。

## 提及人名（mentioned_names 字段）

除 facts 外，顶层再输出 `mentioned_names`：**本场对话中被明确提及的人名数组**——不论
是否有画像事实支撑。对话里顺口问到的人（如「振州那个更新了吗」）没有可抽的事实，
但名字本身值得进待确认队列。规则：
- 同样遵守上面的「人物名字规则」（单个人名、去词缀、非代词、≤8 字、拿不准不收）；
- 已在「已知人物列表」里的人也照收（Go 侧按名去重，不会重复建）；
- 本场没有任何明确人名则省略该字段或给空数组。
...
没有可抽取的信息时输出 {"facts": [], "mentioned_names": []}（若本场有提及人名，mentioned_names 照给）。
...
- pet 平面字段（宠物：提到谁家的宠物就抽一条，subject 是宠物主人；同一只宠物多次提到只抽新信息）：
  ...
  - gender：公|母（**对话明确说了才填**，如「公猫/母猫/他是公的/是母的」；**没说就留空**——绝不根据「他/她」等代词、名字或常见印象猜测，不知道性别就空着，不要默认填「公」）
