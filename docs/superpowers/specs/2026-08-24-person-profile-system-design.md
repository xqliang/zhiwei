...
- **F5 人物归档不级联**：dismiss 人物后其属性/关系行不变，可能留孤儿引用。跟进：级联处置。✅ 已于 P5 解决（2026-08-26）：反向引用（对端关系边/event related_person_ids）刻意不级联——归档不篡改对端人物画像；确认队列可能出现指向已归档人物的 pending 反向边。
  - **级联恢复（2026-08-26 追加，000011）**：六平面各加 `pre_dismiss_status` 列——删除级联 dismiss 时记行原状态，手动删的行该列为 NULL。恢复人物（dismissed→active）在同一事务把级联行翻回**原状态**并清标记；手动删过的行不被误恢复。UI 侧「归档」更名「删除」，名册底部加「已删除」折叠区（`GET /api/persons?dismissed=1` + 恢复按钮）。
