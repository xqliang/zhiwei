...
GET/PATCH/DELETE   /api/persons/{id}                  详情 / 改名·关联声纹·设owner·状态 / 删除（软删+级联，可恢复）
...
- **F4 枚举/类型值写入端不校验**：EnumOptions 与 bool/date 规范形态对 LLM/手动路径均为建议性。跟进：写入校验与规范化。✅ 已于 P6 解决（2026-08-26）：新增 catalog 单点校验 `NormalizeAttrValue`（`internal/profile/validate.go`），按 ValueType 校验并规范化 enum/bool/date/number——LLM 路径违规即 skip（宁少勿错，脏值不落库不进队列，后续会话抽到规范值仍可落），手动路径违规回 400 附具体原因；前端配套 `GET /api/profile/catalog` + 按类型受控输入（enum/bool 下拉、date 日期选择器），受控值天然合法。
- **F5 人物归档不级联**：dismiss 人物后其属性/关系行不变，可能留孤儿引用。跟进：级联处置。✅ 已于 P5 解决（2026-08-26）正向六平面级联；P6 补充（2026-08-26）：**pending 反向关系边**（他人指向已归档人物、待确认的边）一并级联 dismissed，清确认队列孤儿噪声。**active 反向边仍刻意不动**——那是对端人物画像，归档本人不篡改对端；**event related_person_ids 是 JSON 列无法索引级联**，指向已归档人物的事件引用留跟进。
  - **级联恢复（2026-08-26 追加，000011）**：六平面各加 `pre_dismiss_status` 列——删除级联 dismiss 时记行原状态，手动删的行该列为 NULL。恢复人物（dismissed→active）在同一事务把级联行翻回**原状态**并清标记（含 P6 的反向 pending 边对称恢复）；手动删过的行不被误恢复。UI 侧「归档」更名「删除」，名册底部加「已删除」折叠区（`GET /api/persons?dismissed=1` + 恢复按钮）。
- **P2a 简化（2026-08-25 final review 记录）**：① event 的 importance 用 confidence 代偿（手动路径 1.0），独立重要度建模留后续；② related_person_ids MVP 存单元素或空（多人事件取最主要者）；③ 事件自然键对 title 原始精确匹配（不归一化），LLM 标题字面近重复会建两条 active 而非佐证。✅ ③ 已于 P6 解决（2026-08-26）：镜像 attribute 平面的 NormalizeTitle reaffirm 模式——`FindActiveByNormalizedTitleExt`（`internal/repo/person_event.go`）按（主体, 类型, active）拉候选、Go 侧 NormalizeTitle 归一化命中即走 Reaffirm（同值佐证），factKey 同步归一化（`internal/profile/extractor.go`，与 DB 自然键一致，防跨窗口近重复漂移）；近重复标题走佐证而非重复建 active 行。✅ ①② 已于 P7 解决（2026-08-26）：① importance 脱离 confidence 代偿独立建模——取值链「LLM 显式值 > 类型默认」（`eventImportanceOrDefault`/`defaultImportance`，`internal/profile/service.go`：里程碑/成就 0.9、挫折/负面/健康 0.8、旅行/聚会/会议 0.5、其他 0.4；不再用 confidence 代偿），手动路径 `ManualAddEvent`/API `AddEvent` 加可选 `importance`（0/缺省=类型默认，>0 clamp 到 (0,1]），prompt v3 event 段加 importance 字段；② related_person_ids 支持多元素——LLM 路径新键 `related_people` 数组逐个 `resolveSubject`（单人解析失败跳过不阻断事件，见 `applyEventFact`/`Fact.EventRelated`，数组空回退旧 `related` 单人向后兼容），手动路径 `ManualAddEvent` 收 `relatedPersonIDs []ids.ID`、API `AddEvent` body 以 `related_person_ids` 数组为主并入旧单字段 `related_person_id`，前端加事件表单同行人物多选 + 大事记按 importance 视觉分层（`web/app.js`、`web/index.html`）。
- **F6 小项**：ListPending 的 supersedes CurrentValue 查询 N+1（队列规模小，可接受）；属性自然键 SQL 用原始 value_text 精确比较（ParseFacts 已 trim，轻微出入）；集成测试共享 user_id=1 靠 t.Cleanup 维持隔离（可改独立 user_id）。
  - **测试并行隔离（P6 部分，2026-08-26）**：雪花 **nodeID 撞库已解决**——各测试进程改用基于 PID 的互异 node（`ids.InitForTest()` = `pid % 1024`，见 `internal/ids/testinit.go`），消除「多包并行时同 nodeID=1、同毫秒同 step 生成相同雪花 ID、撞共享 `zhiwei_test` 主键」。**但完整并行（`go test ./...` 不带 `-p 1`）仍未解锁**——`-p 1` 暂留的真实原因是另两个**与 nodeID 无关的独立根因**：① `pipeline` 的 Pool 用 `repo.ClaimNext`（`SELECT id FROM pipeline_job WHERE status='pending' ORDER BY id LIMIT 1`）全局领最老 pending job，不按 session/user 限定，并行包二进制互相抢占任务（Makefile 注释早有记录）；② extract 按归一化标题对 `user_id=1` 跨 session 佐证/去重 memory，并行包造同名 active memory 互相污染断言。两者各需独立处理（job 领取按 owner 限定 / 每包或每测试独立 user_id 或独立库），**完整并行解锁见下方 P8 条目**。
  - **完整并行已于 P8 解决（2026-08-26）**——测试库**按包隔离**：`repotest.DSN(t)` 单点把 DSN 库名换成 `zhiwei_test_<调用方包名>`（`runtime.Caller` 取测试文件所在目录名，34 个调用方零逻辑改动（仅机械替换 import/调用点）），首次遇到某库自动 `CREATE DATABASE` + golang-migrate **进程内迁移**（内嵌 `migrations.FS`，与 Makefile 的 migrate CLI 同源 v4、`schema_migrations` 双向兼容），包级 `sync.Map`/`sync.Once` 缓存保证每库只建一次。各测试二进制（Go 包）从此各用各的库（`zhiwei_test_repo` / `_api` / `_pipeline` / `_profile` …），`go test ./...` 并行跑包时互不可见——上面两个根因（ClaimNext 全局抢 pending job、extract user_id=1 跨包污染）被**库级隔离**一并消解。**`-p 1` 已从 Makefile 移除**（`init-testdb` 改为逐个 DROP 所有 `zhiwei_test_*` 库 + 授 `` `zhiwei_test_%`.* `` 通配符权限，MySQL 通配符 GRANT 对尚不存在的库亦生效）；**nodeID 隔离（P6）+ 库隔离（P8）双保险**。**生产二进制零新依赖**——建库/迁移设施独立成 `internal/repotest` 包（`testdb.go`），仅被各包 `_test.go` 引用，`cmd/zhiwei-server` 不链入 golang-migrate/内嵌 SQL。验收：`make init-testdb` 清库后 `TEST_MYSQL_DSN=… go test ./... -count=1`（不带 `-p 1`）连跑 3 轮全绿，且中间不 re-init 的脏库连跑亦绿（包内 `t.Cleanup` 纪律仍在）。
