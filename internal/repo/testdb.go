...
// 函数内直接调 repo.TestDSN(t)，故调用方即该测试文件，其目录名恰为包名（如
// internal/repo → repo → zhiwei_test_repo）。这是有意的零调用方改动设计（34 个
// 测试文件一律不动）；把这处 runtime.Caller 魔法与建库/迁移集中在本文件并配注释，
// 调用深度由 TestCallerPkgDBName 单测锁定（见 testdb_test.go）。
//
...
func TestDSN(t *testing.T) string {
	...
	// Caller(2)：本函数(0) → callerPkgDBName(1) 里的 runtime.Caller 视角下，
	// 从 callerPkgDBName 起 skip=2 恰为 TestDSN 的调用方（测试文件）。
...
// 进程内只跑一次；即便并行子测试同时首调 TestDSN 也只有一个 goroutine 真正建库。
