...
type PersonAttribute struct {
	...
	// PreDismissStatus 归档级联 dismiss 前的状态（active|pending）；NULL=非级联（手动删/正常行）。
...
// DismissAllByPersonExt 人物删除级联（spec §13 F5）：把该人物在**属性平面**上所有活跃态
...
func (r *PersonAttributeRepo) DismissAllByPersonExt(ctx context.Context, ext ExecerContext, personID ids.ID) (int64, error) {
	res, err := ext.ExecContext(ctx,
		`UPDATE person_attribute SET pre_dismiss_status = status, status = 'dismissed' WHERE person_id = ? AND status IN ('active','pending')`,
		personID.Int64())
	if err != nil {
		return 0, err
	}
...
func (r *PersonAttributeRepo) RestoreArchivedExt(ctx context.Context, ext ExecerContext, personID ids.ID) (int64, error) {
	res, err := ext.ExecContext(ctx,
		`UPDATE person_attribute SET status = pre_dismiss_status, pre_dismiss_status = NULL WHERE person_id = ? AND status = 'dismissed' AND pre_dismiss_status IS NOT NULL`,
		personID.Int64())
	if err != nil {
		return 0, err
	}
