...
func TestManualAddAttributeF4(t *testing.T) {
	...
	// 脏枚举 → 报错（哨兵命中），且不落库
	_, err := svc.ManualAddAttribute(ctx, 1, oid, "gender", "男性")
	...
	// 脏日期 → 报错
	if _, err := svc.ManualAddAttribute(ctx, 1, oid, "birthday", "八月三号"); err == nil || !errors.Is(err, ErrInvalidAttrValue) {
		t.Fatalf("脏日期应报 ErrInvalidAttrValue: %v", err)
	}

	// 合法枚举 → 落库
	row, err := svc.ManualAddAttribute(ctx, 1, oid, "gender", "男")
	if err != nil {
	...
	// 合法但需重排的日期 → 落库时已规范化
	row2, err := svc.ManualAddAttribute(ctx, 1, oid, "birthday", "2026/08/03")
	if err != nil {
